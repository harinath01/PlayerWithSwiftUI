...
struct VideoSlider: View {
        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                Rectangle()
                    ...
                    .frame(width: CGFloat(
                        self.calculateProgressWidth(value: bufferedDuration, geometry: geometry)), height: 2.5)
                
                Rectangle()
                    .foregroundColor(Color.red)
                    .frame(width: CGFloat(self.calculateProgressWidth(value: currentTime, geometry: geometry)), height: 2.5)
                ...
                Circle()
                    ...
                    .offset(x: isDragging ? self.draggedLocation! : self.calculateProgressWidth(value: currentTime, geometry: geometry), y: 0)
                    .gesture(DragGesture()
                        .onChanged { value in
    ...
    private func calculateProgressWidth(value: Float64, geometry: GeometryProxy) -> CGFloat {
        let totalWidth = geometry.size.width
        let percentage = CGFloat(value / self.videoDuration)
